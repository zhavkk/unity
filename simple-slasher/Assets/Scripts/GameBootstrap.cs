...
public class GameBootstrap : MonoBehaviour
{
    ...
    private void Awake()
    {
        groundMaterial = MaterialUtility.CreateLitMaterial(groundColor);
        playerMaterial = MaterialUtility.CreateLitMaterial(playerColor);
        enemyMaterial = MaterialUtility.CreateLitMaterial(enemyColor);
        swordMaterial = MaterialUtility.CreateLitMaterial(swordColor);
        fallbackMaterial = MaterialUtility.CreateLitMaterial(new Color(0.4f, 0.4f, 0.4f, 1f));
    }
    ...
    private void TryAutoAssignRpgHeroAssets()
    {
        ...
        {
            playerModelPrefab = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Prefabs/RPGHeroPBR.prefab");
        }

        if (enemyModelPrefab == null)
        {
            enemyModelPrefab = AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Prefabs/Zombie.prefab");
        }
    ...
    private GameObject EnsurePlayer()
    {
        ...
        RageSystem rage = player.GetComponent<RageSystem>();
        if (rage == null)
        {
            rage = player.AddComponent<RageSystem>();
        }
        ...
        SwordSwing swordSwing = EnsureSword(player);
    ...
    {
        PlayerController existingController = Object.FindAnyObjectByType<PlayerController>();
    ...
    private void EnsureUI(GameObject player)
    {
        ...
        if (rageFill != null)
        {
            RageBarUI rageUi = ragePanel.GetComponent<RageBarUI>();
            if (rageUi == null)
            {
                rageUi = ragePanel.gameObject.AddComponent<RageBarUI>();
            }
        ...
        if (healthFill != null)
        {
            HealthBarUI healthUi = healthPanel.GetComponent<HealthBarUI>();
            if (healthUi == null)
            {
                healthUi = healthPanel.gameObject.AddComponent<HealthBarUI>();
            }
    ...
    private SwordSwing EnsureSword(GameObject player)
    {
        ...
        {
            SwordSwing existingSwing = existingSword.GetComponent<SwordSwing>();
            if (existingSwing == null)
            {
                existingSwing = existingSword.gameObject.AddComponent<SwordSwing>();
            }
        ...
        SwordSwing swing = pivot.GetComponent<SwordSwing>();
        if (swing == null)
        {
            swing = pivot.gameObject.AddComponent<SwordSwing>();
        }
    ...
    private void FixErrorShaderMaterials()
    {
        ...
        foreach (Renderer renderer in renderers)
        {
            ...
            for (int i = 0; i < materials.Length; i++)
            {
                ...
                if (MaterialUtility.NeedsUrpUpgrade(material))
                {
                    materials[i] = MaterialUtility.GetOrCreateUrpLit(material, fallbackMaterial);
                    changed = true;
                }
            }
