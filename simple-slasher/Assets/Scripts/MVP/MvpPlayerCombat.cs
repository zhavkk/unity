...
public class MvpPlayerCombat : MonoBehaviour
{
    [Header("Attack")]
    ...
    [SerializeField] private MvpSwordSwing swordSwing;
    ...
    public void Initialize(MvpRageSystem rage, Transform attackPointTransform, MvpPlayerHealth health, MvpSwordSwing swing)
    {
        rageSystem = rage;
    ...
    private void PerformAttack()
    {
        ...
        HashSet<MvpEnemy> damaged = new HashSet<MvpEnemy>();
        ...
        foreach (Collider hit in hits)
        {
            ...
            MvpEnemy enemy = hit.GetComponentInParent<MvpEnemy>();
    ...
    {
        MvpEnemy target = FindClosestEnemy(dashRange);
        if (target == null)
        {
            return;
        }
    ...
    {
        MvpEnemy target = FindClosestEnemy(pullRange);
        if (target == null)
        {
            return;
        }
    ...
    private MvpEnemy FindClosestEnemy(float range)
    {
        MvpEnemy[] enemies = Object.FindObjectsByType<MvpEnemy>(FindObjectsSortMode.None);
        MvpEnemy closest = null;
        float closestDistance = range;

        foreach (MvpEnemy enemy in enemies)
        {
            if (enemy == null)
            {
                continue;
